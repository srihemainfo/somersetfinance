...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.personalDetail.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.personal-details.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('aadhar_number') ? 'is-invalid' : '' }}" type="text" name="aadhar_number" id="aadhar_number" value="{{ old('aadhar_number', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('mother_tongue') ? 'is-invalid' : '' }}" name="mother_tongue_id" id="mother_tongue_id">
                    @foreach($mother_tongues as $id => $entry)
                        <option value="{{ $id }}" {{ old('mother_tongue_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
