                                ...
                                <tr>
                                    <th style="width: 30%;" >
                                        <div class="form-group ">
                                            <input type="text" style="width: 100%;" class="form-control" name="purchase_titles[]" value="{{ $purchase->purchase_title }}" required>
                                            <input type="hidden" class="form-control" name="purchase_id[]" value="{{ $purchase->id }}" required>
                                        </div>
                                    </th>
                                    <th style="width: 30%;" >
                                        <div class="form-group ">
                                            <input type="text" class="form-control" name="purchase_price[]" value="{{ old('purchase_price') ?? $purchase->purchase_price }}" required oninput="this.value = this.value.replace(/[^\d.]/g, '');">

                                            {{-- <input type="text" class="form-control" name="purchase_price[]" value="{{ old('purchase_price') ?? $EvetList->purchase_price }}" required oninput="this.value = this.value.replace(/[^0-9]/g, '');"> --}}
                                        </div>
                                    </th style="width: 30%;" >
    ...
    document.addEventListener("DOMContentLoaded", function () {
        document.getElementById("addRowBtn").addEventListener("click", function () {
            ...
            <tr>
                <td style="width: 30%;" >
                    <div class="form-group" >
                        <input type="text" class="form-control" name="purchase_titles[]" value="" required>
                        <input type="hidden" class="form-control" name="purchase_id[]" value="" required>
                    </div>
                </td>
                <td style="width: 30%;" >
                    <div class="form-group">
                        <input type="text" class="form-control" name="purchase_price[]" value="" required oninput="this.value = this.value.replace(/[^0-9]/g, '');">
                    </div>
                </td>
                <td style="width: 30%;" ><button class='btn btn-danger' type="button" onclick="removeRow(this)">Remove</button></td>
            </tr>`;
