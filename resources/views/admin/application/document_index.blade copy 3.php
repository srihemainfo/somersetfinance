    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '#submit_button', function(e) {
            ...
            $('#uploaded_images_container form').each(function() {
                ...
                // console.log($('#uploaded_images_container form').length);
            ...
            let data = [];
            // $("input[name='input_remark[]']").each(function() {
            //     data.push($(this).val());
            // });
            ...
            // $('#uploaded_images_container').find('.uploaded_file').each(function(index, element) {
            //     const fileId = $(element).attr('id').split('_').pop();
            //     const remarkInput = $(`#input_remark_${fileId}`).val();

            //     if (selectedFiles[index]) {
            //         selectedFiles[index].remark = remarkInput;
            //         formData.append('files[]', selectedFiles[index].file);
            //         formData.append('remarks[]', selectedFiles[index].remark);
            //         formData.append('uploaded_files[]', selectedFiles[index].uploaded_file);
            //     }
            // });
        ...
        if (applicationId == undefined || documentId == undefined ) {
                ...
                const checkbox = $('<input>', { type: 'checkbox', class: 'delete-checkbox', 'data-file-id': `uploaded_file_${index + 1}` });
        ...
        function createNewAddedContent(index) {
            const newContent = $('<div>', { class: 'container-fluid', id: `new_added_content_${index}` });

            const pane = $('<div>', { class: 'pane m-3' });
            const idkLabel = $('<label>', { class: 'label' }).html(`<span>Idk</span><input id="idk_${index}" class="input idk" name="radio" type="radio" checked="checked">`);
            const yesLabel = $('<label>', { class: 'label' }).html(`<span>Yes</span><input id="yes_${index}" class="input yes" name="radio" type="radio">`);
            const noLabel = $('<label>', { class: 'label' }).html(`<span>No</span><input id="no_${index}" class="input no" name="radio" type="radio">`);
            ...
            const extraText = $('<input>', { type: 'text', id: `extra-text_${index}`, class: 'form-control', placeholder: 'Reason' });
        ...
        $(document).on('click', '[id^=upload_preview_]', function(event) {
            ...
            modalBody.append(createNewAddedContent(index));
