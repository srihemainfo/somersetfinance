...
input[type=checkbox] {
    ...
    <section class="content">
        ...
        @php
            ...
            $status = $data->status;
            $statuses = [
                'Completed' => ['enquiry' => 'completed', 'underwriting' => 'completed', 'processing' => 'completed', 'submitted' => 'completed', 'complete' => 'completed'],
                'Submitted' => ['enquiry' => 'completed', 'underwriting' => 'completed', 'processing' => 'completed', 'submitted' => 'current', 'complete' => 'upcoming'],
                'Processing' => ['enquiry' => 'completed', 'underwriting' => 'completed', 'processing' => 'current', 'submitted' => 'upcoming', 'complete' => 'upcoming'],
                'Underwriting' => ['enquiry' => 'completed', 'underwriting' => 'current', 'processing' => 'upcoming', 'submitted' => 'upcoming', 'complete' => 'upcoming'],
            ];
            
            $default = ['enquiry' => 'upcoming', 'underwriting' => 'upcoming', 'processing' => 'upcoming', 'submitted' => 'upcoming', 'complete' => 'upcoming'];
            
            $values = $statuses[$status] ?? $default;
            ...
            <div class="row my-3">
                <div class="col-sm-12">
                    <div class="stage_view d-flex justify-content-evenly table-responsive customized_scrollbar">
                        <div class="stage {{ $enquiry }}  enquiry"></div>
                        <div class="stage  {{ $underwriting }}  underwriting"></div>
                        <div class="stage {{ $processing }}  processing"></div>
                        <div class="stage {{ $submitted }}  completion"></div>
            ...
            <div class="row my-3">
                <div class="col-sm-6">
                    <div class="profile_view">
                        <h5>{{ $data->customerdetail->name ?? 'N/a' }}</h5>
                        <p>
                ...
                <div class="col-sm-6">
                    <div class="profile_view text-right">
                        <h5>Case No: <span id="case_number" class="text-uppercase" title="Click to copy">{{ $data->ref_no ?? 'N/A' }}</span></h5>
                        <p>
        ...
        <div class="container mt-4">
                                ...
                                <div class="row">
                                    <div class="text-left row">
                                        ...
                                        <div class="col-sm-6 text-right">
                                            <a title="Edit Customer Details" id='applicant_edit' data-applicant-id="{{ $data->customerdetail->id ?? 'N/a' }}" data-applicant-name="{{ $data->customerdetail->name ?? 'N/a' }}">
                            ...
                            <div class="card-header">
                                <div class="row">
                                    <div class="text-left row">
                                        ...
                                        @if($roleTitle == 1)
                                        <div class="col-sm-6 text-right">
                                            <a href="{{ route('admin.application-stage.edit',$data->id) }}" title="Edit Loan Details">
                                                <i class="far fa-edit"></i>
                                            </a>
                                        </div>
                                        @endif
                                ...
                                <div class="table-responsive">
                                    <table class="table table-borderless mb-0 profile_details">
                                        <tbody>
                                            ...
                                            <tr>
                                                <th class="ps-0" scope="row">Term Year :</th>
                                                <td class="text-muted">{{$data->term_year ?? 'N/a' }}</td>
                                            </tr>
                                            <tr>
                                                <th class="ps-0" scope="row">Term Month:</th>
                                                <td class="text-muted">{{$data->term_month ?? 'N/a' }}</td>
                                            </tr>
                                            <tr>
            ...
            <!--</div>-->
             @if((isset($data->applicationLoanDocument2)  && count($data->applicationLoanDocument2) > 0) || ( isset($data->applicantDocument1)  && count($data->applicantDocument1) > 0 ) || (isset($data->additionalDocument2) && count($data->additionalDocument2) >0 ) || (isset($data->applicantformUpload2)  && count($data->applicantformUpload2) > 0 )|| ( isset($data->applicationLoanFormUpload2)  && count($data->applicationLoanFormUpload2) > 0 ) || (isset($data->additionFormUploads) && count($data->additionFormUploads) > 0 ))
                   
                <div class="row mt-3">
                        @if((isset($data->applicationLoanDocument2)  && count($data->applicationLoanDocument2) > 0) || ( isset($data->applicantDocument1)  && count($data->applicantDocument1) > 0 ) || (isset($data->additionalDocument2) && count($data->additionalDocument2) >0 ))
                            <div class="col-md-6">
                                        ...
                                        <div class="card-body table-responsive">
                                            <table class="table table-bordered">
                                                ...
                                                <tbody>
                                                    ...
                                                    @php
                                                        ...
                                                        $documentImagesForDocument = $documentImages->where('document_id', $document->id);
                                                            ...
                                                            $verifiedCount = $documentImagesForDocument->where('status', 'Verified')->count();
                                                        @endphp
                                                            <tr>
                                                                <th scope="row">{{ $si }}</th>
                                                ...
                                                @php
                                                    ...
                                                    $verifiedCount = $documentImagesForDocument->where('status', 'Verifed')->count();
                                                    $rejectedCount = $documentImagesForDocument->where('status', 'Rejected')->count();
                                                    $uploadCount = $documentImagesForDocument->where('status', 'Upload')->count();
                                                     $reuploadCount = $documentImagesForDocument->where('status', 'Reupload')->count();
                                                @endphp
                    ...
                    @if((isset($data->applicantformUpload2)  && count($data->applicantformUpload2) > 0 )|| ( isset($data->applicationLoanFormUpload2)  && count($data->applicationLoanFormUpload2) > 0 ) || isset($data->additionFormUploads) && count($data->additionFormUploads) > 0 )
                        <div class="col-md-6">
                                <div class="documents_view">
                                    <div class="card">
                                        <div class="card-header">
                                                ...
                                                <tbody>
                                                    ...
                                                    @if (isset($data->additionFormUploads) && count($data->additionFormUploads) > 0 )
                                                        ...
                                                        @foreach($data->additionFormUploads as $document)
                                                            @php
                                                                if($data->ApplicationAdditionFormUploads){
                                                                    ...
                                                                    $verifiedCount = $data->ApplicationAdditionFormUploads->where('status', 'Verified')->count();
                                                                }else{
             ...
             @php
                $stages = ['Processing', 'Submitted', 'Completed','Canceled'];
            @endphp
        
        @if(($roleTitle == 1 || $roleTitle != 1 ) && in_array($status, $stages) && (isset($data->applicationLoanDocument2)  && count($data->applicationLoanDocument2) > 0) || ( isset($data->applicantDocument1)  && count($data->applicantDocument1) > 0 ) || (isset($data->additionalDocument2) && count($data->additionalDocument2) >0 ) || (isset($data->applicantformUpload2) && count($data->applicantformUpload2) > 0 )|| ( isset($data->applicationLoanFormUpload2)  && count($data->applicationLoanFormUpload2) > 0 ) || isset($data->additionFormUploads) && count($data->additionFormUploads) > 0)
            ...
            <div class="card-body row" style="gap:50px;">
                ...
                <div class="form-group col-sm-4 col-md-4 col-xl-4 col-12">
                   <label for="Loan_type_filter" class="col-form-label"> Change the Case Stage</label>
                        @php
                           $stages = ['Canceled','Submitted'];
                             $condition = false; 
                            if( isset($data->status) && in_array($status, $stages)){
                               $condition = true; 
                            }
                         
                        @endphp
                    <select class="form-control" name="caseStage" id="caseStage">
     ...
     @endif
        ...
        @if(
            (isset($data->applicationLoanDocument2) && count($data->applicationLoanDocument2) == 0) &&
            (isset($data->applicantDocument1) && count($data->applicantDocument1) == 0) &&
            (isset($data->additionalDocument2) && count($data->additionalDocument2) == 0) &&
            (isset($data->applicantformUpload2) && count($data->applicantformUpload2) == 0) &&
            (isset($data->additionFormUploads) && count($data->additionFormUploads) == 0)
        )
...
<script>
    ...
    $(document).ready(function() {
        ...
        handleRemarkSubmit('.remark_submit', "{{ route('admin.document.remarkUpdatedadmin') }}", 'applicationId', 'documentId');
        ...
        // Initialize submit button handlers
        handleSubmitButtonClick('#submit_button', "{{ route('admin.document.remarkUpdated') }}", 'applicationId', 'documentId', '#uploaded_images_container');
    ...
    function handleModalUploadClick(buttonSelector, ajaxUrl, modalId, containerSelector, applicationIdKey, documentIdKey, modalApplicationIdField, modalDocumentIdField) {
        $(document).on('click', buttonSelector, function() {
            ...
            formData.append('documentId', documentId);
                ...
                $('.secondLoader').show();
                $('#uploaded_images_container').empty();
                $('#uploaded_images_container1').empty();
                $('#uploaded_images_container2').empty();
                $('#uploaded_additional_images_container').empty();
    ...
    handleModalUploadClick('.upload_document_modal', "{{ route('admin.document.upload_image_get') }}", '#upload_document_modal', '#uploaded_images_container', 'applicationId', 'documentId', '#modal_application_id', '#modal_document_id');
    handleModalUploadClick('.upload_additional_document_modal', "{{ route('admin.document.upload_additional_image_get') }}", '#additional_modal', '#uploaded_images_container2', 'applicationId2', 'documentId2', '#modal_application_id2', '#modal_document_id2');
    handleModalUploadClick('.upload_form_modal', "{{ route('admin.document.upload_form_image_get') }}", '#upload_document_modal_1', '#uploaded_images_container1', 'applicationId1', 'documentId1', '#modal_application_id1', '#modal_document_id1');
    handleModalUploadClick('.upload_additional_form_modal', "{{ route('admin.document.upload_additional_form_image_get') }}", '#upload_additional_form_modal', '#uploaded_additional_images_container', 'applicationId3', 'documentId3', '#modal_additional_application_id', '#modal_additional_document_id');
    ...
    handleFileUpload('#file_upload', '#uploaded_images_container', "{{ route('admin.document.upload_image_get') }}", 'applicationId', 'documentId');
    ...
    handleFileUpload('#file_upload3', '#uploaded_additional_images_container', "{{ route('admin.document.upload_additional_form_image_get') }}", 'applicationId3', 'documentId3');
...
$(document).on('click', '[id^=upload_change_], [id^=upload1_change_], [id^=upload2_change_],[id^=upload3_change_]', function(event) {
    ...
    if (buttonId.startsWith('upload1_change_')) {
        ...
        ajaxUrl = "{{ route('admin.document.upload_additional_form_image_get') }}";
        number = 3;
    }
...
// function handleDeleteClick(deleteButtonId, checkboxClass, url, applicationIdKey, documentIdKey) {
...
handleDeleteClick('#delete_selected', '.delete-checkbox', "{{ route('admin.document.upload_image_get_deletes') }}", 'applicationId', 'documentId');
handleDeleteClick('#delete_selected1', '.form_delete-checkbox', "{{ route('admin.document.upload_form_image_get_deletes') }}", 'applicationId1', 'documentId1');
handleDeleteClick('#delete_selected2', '.addition_delete_checkbox', "{{ route('admin.document.upload_additional_image_get_deletes') }}", 'applicationId2', 'documentId2');
handleDeleteClick('#delete_selected3', '.form_additional_delete-checkbox', "{{ route('admin.document.upload_form_additional_image_get_deletes') }}", 'applicationId3', 'documentId3');
...
$(document).ready(function() {
    $('#case_number').click(function() {
        var $temp = $("<input>");
        $("body").append($temp);
        $temp.val($(this).text()).select();
        document.execCommand("copy");
        $temp.remove();
        alert('Case number copied: ' + $(this).text());
    });
});

$(document).on('change', '#caseStage', function(event) {
    event.preventDefault();

    const status = $(this).val();
    const case_id = {{ $data-> id ?? ''  }} ;
    let data = { status : status, id : case_id}

    if(data && case_id ){
    
        Swal.fire({
            title: "Are You Sure?",
            text: "Do You Want to Chage the Case Stage?",
            icon: "warning",
            showCancelButton: true,
            confirmButtonText: "Yes",
            cancelButtonText: "No",
            ...
            if (result.value) {
                ...
                $.ajax({
                    url: "{{ route('admin.document.case_status_change') }}",
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    ...
                    success: function(response) {
                        if (response.status) {
                            ...
                            Swal.fire("Error", response.message || "Failed to Status Change", "error");
                        }
                        loader.hide();
                    },
                    error: function() {
