...
class ModelController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $brands = Brand::pluck('brand','id')->prepend('Select Brand', '') ;
        ...
        if ($request->ajax()) {
            ...
            // $query = Models::query(); // Start building the query from the Models model
            // $table = Datatables::of($query);

            // If you want to select all columns from the Models table
            // $table->select('*');
            ...
            $table->editColumn('actions', function ($row) {
                ...
                $viewGate = 'model_show'; // need to change gate permission
                $editGate = 'model_edit';
                $deleteGate = 'model_delete';
    ...
    {
        if (isset($request->model) &&  isset($request->brand_id) ) {
            if ($request->id == '') {
                $count = Models::where(['model' => $request->model , 'brand_id' => $request->brand_id])->count();
                if ($count > 0) {
